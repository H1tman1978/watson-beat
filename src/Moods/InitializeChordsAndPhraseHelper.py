...
def InitializeChordsAndPhraseForSections(self, sections):
    ...
    for secId in sections:
        ...
        self.sections[secId]['chords'], self.sections[secId]['phrases'], globalTick = getChordSettingsForSection(self,
                                                                                                                 secId,
                                                                                                                 energy,
                                                                                                                 direction,
                                                                                                                 slope,
...
def getChordSettingsForSection(self, secId, energy, direction, slope, uniqCPId, globalTick, mustHaveLayers,
                               mustNotHaveLayers):
    ...
    if (energy == 'demo'):
        ...
        currNumLayers = random.randint(self.MaxAndMinLayersForEnergy[energy]['initialMin'],
                                       self.MaxAndMinLayersForEnergy[energy]['initialMax'])
    ...
    approxNumChordsInSection = numPhrasesInSection * len(
        chordSeq)  # why approximate, because the last phrase may not be a complete phrase
    ...
    if (1):
        print()
        print("Initial.", "Section id: ", secId, "Energy: ", energy, "Direction: ", direction, "slope: ", slope,
              "Slope Multiplier: ", multiplier, "AddLayers: ", addLayers, "Final Adder: ", adder, "curr density: ",
              currNumLayers)
    ...
    while (currTick < totalTicks):
        ...
        for chordTick in chordSeq:
            ...
            if (0):
                print("Sec Id: ", secId, "chordTick: ", chordTick, "currtick: ", currTick, "totalTicks: ", totalTicks,
                      "phraseId: ", phId, "chId: ", chId, "numChordsInPhrase: ", numChordsInPhrase)

            if (currTick >= totalTicks):
                firstChordInPhrase = numChordsInPhrase * (phId)
                if (0):
                    print("I am here 1")
                    print("chId: ", chId, "phId: ", phId, "firtsChordInPhrase: ", firstChordInPhrase, "num chords: ",
                          numChordsInPhrase)

                if (similarId != -1):
                    phLayers = self.sections[similarId]['phrases'][phId]['layers']

                phrases[phId] = {'layers': list(set(phLayers)), 'globalEndTick': chords[chId - 1]['globalEndTick'],
                                 'globalStartTick': chords[firstChordInPhrase]['globalStartTick'],
                                 'startMNum': startMNum, 'endMNum': startMNum + pl - 2, 'density': density}
                phLayers = []
                phId += 1
                startMNum += pl
                break
            ...
            if (chordTick + currTick <= totalTicks):
                ...
                chords[chId] = {'startTick': currTick, 'endTick': chordTick + currTick, 'layers': layers,
                                'duration': chordTick, 'density': density, 'globalStartTick': currTick + globalTick,
                                'globalEndTick': chordTick + currTick + globalTick}
                currTick += chordTick
                chId += 1
                firstChordInPhrase = numChordsInPhrase * (phId)
                if (0):
                ...
                if (chId % numChordsInPhrase == 0):  # account for the layers for the phrase
                    ...
                    phrases[phId] = {'layers': list(set(phLayers)), 'globalEndTick': chords[chId - 1]['globalEndTick'],
                                     'globalStartTick': chords[firstChordInPhrase]['globalStartTick'],
                                     'startMNum': startMNum, 'endMNum': startMNum + pl - 1, 'density': density}
                    phLayers = []
                    phId += 1
                    startMNum += pl
                ...
                chords[chId] = {'startTick': currTick, 'endTick': diff + currTick, 'layers': layers, 'duration': diff,
                                'density': density, 'globalStartTick': currTick + globalTick,
                                'globalEndTick': diff + currTick + globalTick, }
                currTick += diff
                chId += 1
                firstChordInPhrase = numChordsInPhrase * (phId)
                if (0):
            ...
            if (chordBased):
                ...
                if (0):
                    print("Next Chord.", "Section id: ", secId, "Energy: ", energy, "Direction: ", direction, "slope: ",
                          slope, "Slope Multiplier: ", multiplier, "AddLayers: ", addLayers, "Final Adder: ", adder,
                          "curr density: ", currNumLayers)

                layers, density = getLayersBasedOnEnergy(self, energy, currNumLayers, layers, mustHaveLayers,
                                                         mustNotHaveLayers)
        ...
        layers, density = getLayersBasedOnEnergy(self, energy, currNumLayers, layers, mustHaveLayers, mustNotHaveLayers)
        if (0):
            print("Next Phrase.", "Section id: ", secId, "Energy: ", energy, "Direction: ", direction, "slope: ", slope,
                  "Slope Multiplier: ", multiplier, "AddLayers: ", addLayers, "Final Adder: ", adder, "curr density: ",
                  currNumLayers)
...
def getLayersBasedOnEnergy(self, energy, numLayers, prevLayers, mustHaveLayers, mustNotHaveLayers):
    ...
    if (len(prevLayers) == 0 or len(
            prevLayers) <= numLayers):  # first phrase in section (so empty prevlayer) or go up in density or have the same density
