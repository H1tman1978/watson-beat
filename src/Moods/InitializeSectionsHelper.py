...
from Skeleton import (BassRhythms, Constants)


def InitializeSectionsComplex(self, sections, moodSpecificInfo):
    # get the mood specific info
    fillsFlag = moodSpecificInfo['fills']
    if 'numChords' in moodSpecificInfo:  # this information is passed as parameter to the function
        # getNumChordsAndDurationForCP ( self, cpId, pl, tse, numChords ) :
    ...
    for secId in sections:
        ...
        if ('duration' in sections[secId]):
            ...
            numSecondsForSection, secBPM = findBestFitForSectionTimeGivenBPM(possibleSecondsForSection, possibleBPMs,
                                                                             tse, pl)
        ...
        if (1):
            print("SecId: ", secId, "Chosen BPM: ", secBPM, "TSE: ", tse, "numSecondsForSection: ",
                  numSecondsForSection, "numSecondsPerMeasure: ", numSecondsPerMeasure, "numMeasures: ",
                  numMeasuresInSection, "PL: ", pl, "num seconds per phrase: ", secondsForOnePhrase, "num Phrases: ",
                  numPhrases)
            print()

        if (similarId != -1 and numMeasuresInSection > self.sections[similarId][
            'numMeasures']):  # if the repeating section is longer, make that the same size as the similar section
        ...
        if (similarId == -1):  # unique section not modeled after existing sections
            self.sections[secId] = {'id': secId, 'similarId': -1,
                                    'numSeconds': numMeasuresInSection * numSecondsPerMeasure, 'tse': tse,
                                    'bpm': secBPM, 'pl': self.primaryPL, 'key': self.primaryScale,
                                    'numMeasures': numMeasuresInSection, 'repCount': numPhrases, 'totalTicks': numTicks,
                                    'startMNum': startMNum, 'endMNum': startMNum + numMeasuresInSection - 1,
                                    'percussionFills': fillsFlag}
        else:
            tse = self.sections[similarId]['tse']
            bpm = self.sections[similarId]['bpm']
            self.sections[secId] = {'id': secId, 'similarId': similarId, 'numSeconds': numSecondsForSection, 'tse': tse,
                                    'bpm': bpm, 'pl': self.primaryPL, 'key': self.primaryScale,
                                    'numMeasures': numMeasuresInSection, 'repCount': numPhrases, 'totalTicks': numTicks,
                                    'startMNum': startMNum, 'endMNum': startMNum + numMeasuresInSection - 1,
                                    'percussionFills': fillsFlag}
        ...
        if (0):
            print("SecId: ", secId, "Approx Num Seconds For Section: ", numSecondsForSection,
                  "Actual num seconds per section: ", numMeasuresInSection * numSecondsPerMeasure,
                  "Num Measures In Section: ", numMeasuresInSection, "Num Phrases: ", numPhrases)
    ...
    for uniqCPId in range(self.numUniqCPs):
        ...
        self.uniqCPSettings[uniqCPId] = {'scale': self.primaryScale, 'tse': tse, 'pl': self.primaryPL, 'cpSeq': [],
                                         'numChords': 0,
                                         'percussionSettings': {'BeatInfo': BeatInfo, 'Patterns': Patterns,
                                                                'type': pType}}
    ...
    for cpId in range(self.numUniqCPs):
        self.uniqCPSettings[cpId]['cpSeq'], self.uniqCPSettings[cpId]['numChords'] = getNumChordsAndDurationForCP(self,
                                                                                                                  cpId,
                                                                                                                  self.primaryPL,
                                                                                                                  self.uniqCPSettings[
                                                                                                                      cpId][
...
def findBestFitForSectionTimeGivenBPM(possibleSecondsForSection, possibleBPMs, tse, pl):
    ...
    if (0):
        ...
        print("Chosen Mid Phrases: ", chosenMidPhrases, "num seconds: ", chosenMidDuration, "given seconds: ",
              givenMidDuration)
    ...
    while True:
        ...
        for numSecondsForSection in possibleSecondsForSection:
            ...
            for bpm in possibleBPMs:
                ...
                if (0):
                    print("BPM: ", bpm, "Num seconds for section: ", numSecondsForSection, "Num Seconds Per Measure: ",
                          numSecondsPerMeasure, "Num Seconds for One Phrase: ", numSecondsForOnePhrase,
                          "Num Seconds for Half Phrase: ", numSecondsForHalfPhrase)
...
def InitializeSections(self, sections, moodSpecificInfo):
    ...
    fillsFlag = moodSpecificInfo['fills']
    if (
            'numChords' in moodSpecificInfo):  # this information is passed as parameter to the function  getNumChordsAndDurationForCP ( self, cpId, pl, tse, numChords ) :
        numChords = moodSpecificInfo['numChords']
    else:
        numChords = -1
    ...
    for secId in sections:
        ...
        if (similarId == -1):  # unique section not modeled after existing sections
            self.sections[secId] = {'id': secId, 'similarId': -1,
                                    'totalSeconds': numMeasuresInSection * numSecondsPerMeasure, 'tse': self.primaryTSE,
                                    'bpm': self.primaryBPM, 'pl': self.primaryPL, 'key': self.primaryScale,
                                    'numMeasures': numMeasuresInSection, 'repCount': numPhrases, 'totalTicks': numTicks,
                                    'startMNum': startMNum, 'endMNum': startMNum + numMeasuresInSection - 1,
                                    'melId': uniqId, 'percussionFills': fillsFlag}
        else:
            uniqId = self.sections[similarId]['melId']
            self.sections[secId] = {'id': secId, 'similarId': similarId,
                                    'totalSeconds': numMeasuresInSection * numSecondsPerMeasure, 'tse': self.primaryTSE,
                                    'bpm': self.primaryBPM, 'pl': self.primaryPL, 'key': self.primaryScale,
                                    'numMeasures': numMeasuresInSection, 'repCount': numPhrases, 'totalTicks': numTicks,
                                    'startMNum': startMNum, 'endMNum': startMNum + numMeasuresInSection - 1,
    ...
    for uniqCPId in range(self.numUniqCPs):
        BeatInfo, Patterns, pType = self.setPercussionSettings(self.primaryTSE)
        self.uniqCPSettings[uniqCPId] = {'scale': self.primaryScale, 'bpm': self.primaryBPM, 'tse': self.primaryTSE,
                                         'pl': self.primaryPL, 'cpSeq': [], 'numChords': 0,
                                         'percussionSettings': {'BeatInfo': BeatInfo, 'Patterns': Patterns,
                                                                'type': pType}}
...
def getNumChordsAndDurationForCP(self, cpId, pl, tse, numChords):
    ...
    if (
            numChords == -1):  # if mood specific info does not include number of chords, then determine number of chords here
        if (pl == 2):
            numChords = random.choice([1, 2, 2])
    ...
    while True:
        ...
        if (0):
            print("Try Number: ", tryNum, " Sequence not found: ", "Num Chords: ", numChords, "Sequence: ",
                  cpDurationSeq, "Max Duration: ", maxduration, "curr ticks: ", currTicks)
