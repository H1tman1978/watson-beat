...
def run(BeatInfo, Patterns, tse, rhythmSpeed, pl):
    ...
    SnareDrum[
        'regMeasure'] = collections.OrderedDict()  # this is the normal snare drum pattern for a measure that is not eos or eop
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            SnareDrum['regMeasure'][patternNum].append(
            ...
            SnareDrum['regMeasure'][patternNum].append(
                {'beatNum': beatNum, 'event': 'off', 'note': DrumConstants.fillNotes['mid']['snare'], 'velocity': 0,
                 ...
                 'octave': DrumConstants.fillNoteOctaves['mid']['snare'],
                 'pitch': "midi." + DrumConstants.fillNotes['mid']['snare'] + "_" + str(
                     DrumConstants.fillNoteOctaves['mid']['snare'])})
    ...
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            if (beatNum >= eopStartBeat):  # this beat will be filled in by eopFill
                ...
                SnareDrum['eopMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'on', 'note': DrumConstants.fillNotes['mid']['snare'], 'velocity': 0,
    ...
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            if (beatNum >= eosStartBeat):  # this beat will be filled in by eosFill
                ...
                SnareDrum['eosMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'on', 'note': DrumConstants.fillNotes['mid']['snare'], 'velocity': 0,
                     'midiClk': 0, 'octave': DrumConstants.fillNoteOctaves['mid']['snare'],
                     'pitch': "midi." + DrumConstants.fillNotes['mid']['snare'] + "_" + str(
                         DrumConstants.fillNoteOctaves['mid']['snare'])})
