...
class DrumFill:
    ...
    def __init__(self, tse, startBeat, numBeats, rhythmSpeed, complexity):
        ...
        if (complexity.endswith('simple')):
            maxMovementPoints = random.choice([1, 2, 2,
                                               3])  # this is indicates the number of  movements of the drumsticks , eg: high mid low high
            maxMovementPoints = random.choice([1, 2, 2,
                                               2])  # this is indicates the number of  movements of the drumsticks , eg: high mid low high
            self.maxParallel = 2  # how many notes can play at the same time
        else:
            maxMovementPoints = random.choice(
                [2, 3, 3])  # this is indicates the number of  movements of the drumsticks , eg: high mid low high
            self.maxParallel = 2  # how many notes can play at the same time
        ...
        for i in range(1, maxMovementPoints):
            if (
                    i == maxMovementPoints - 1):  # if this is the last gesture position, assign remaining beats to this position
        ...
        for i in range(maxMovementPoints):
            ...
            if (rhythmSpeed == 'slow'):
                maxDensity = max(1, random.randint(numBeatsForGesturePosition[i] // 4, numBeatsForGesturePosition[
                    i] // 2))  # how crowded should the fill be
            elif (rhythmSpeed == 'medium'):
                maxDensity = max(1, random.randint(numBeatsForGesturePosition[i] // 2, (
                            3 * numBeatsForGesturePosition[i]) // 4))  # how crowded should the fill be
            ...
            self.gesturePosition.append(
                {'position': random.choice(movementTypes), 'numBeats': numBeatsForGesturePosition[i],
                 'maxDensityInBeats': maxDensity, 'fillBeats': fillBeats})
        ...
        if (0):
            print("Tse: ", tse, "StartBeat: ", self.startFillBeat, "NumBeats: ", self.numBeats, "Complexity: ",
                  complexity, "RhythmSpeed: ", rhythmSpeed, "MaxMovementPoints: ", maxMovementPoints,
                  numBeatsForGesturePosition)
            for i in range(maxMovementPoints):
    ...
    def createFills(self):
        ...
        if (currBeatNum != 1):
            # rest note
            fillMeasure[cnt] = {'beatNum': 1, 'event': 'on', 'note': DrumConstants.fillNotes['low']['kick'],
                                'velocity': 0, 'midiClk': 0, 'octave': DrumConstants.fillNoteOctaves['low']['kick'],
                                'pitch': "midi." + DrumConstants.fillNotes['low']['kick'] + "_" + str(
                                    DrumConstants.fillNoteOctaves['low']['kick'])}
            fillMeasure[cnt + 1] = {'beatNum': 1, 'event': 'off', 'note': DrumConstants.fillNotes['low']['kick'],
                                    'velocity': 0,
                                    'midiClk': (currBeatNum - 1) * DrumConstants.SmallestGranularityInTicks,
            ...
            for beat in range(1, gesturePos['numBeats'] + 1):
                ...
                if (beat in gesturePos['fillBeats']):
                    ...
                    for i in range(len(notes)):
                        fillMeasure[cnt] = {'beatNum': currBeatNum, 'event': 'on', 'note': notes[i],
                                            'velocity': velocity[i], 'midiClk': 0, 'octaves': octaves[i],
                                            'pitch': "midi." + notes[i] + "_" + str(octaves[i])}
                        cnt += 1

                    fillMeasure[cnt] = {'beatNum': currBeatNum, 'event': 'off', 'note': notes[0], 'velocity': 0,
                                        'midiClk': DrumConstants.SmallestGranularityInTicks, 'octaves': octaves[0],
                                        'pitch': "midi." + notes[0] + "_" + str(octaves[0])}
                    cnt += 1
                    for i in range(1, len(notes)):
                        fillMeasure[cnt] = {'beatNum': currBeatNum, 'event': 'off', 'note': notes[i], 'velocity': 0,
                                            'midiClk': 0, 'octaves': octaves[i],
                                            'pitch': "midi." + notes[i] + "_" + str(octaves[i])}
                        cnt += 1


                else:  # rest note
                    fillMeasure[cnt] = {'beatNum': currBeatNum, 'event': 'on', 'note': 'C', 'velocity': 0, 'midiClk': 0,
                                        'octaves': 3, 'pitch': "midi.C_3"}
                    cnt += 1
                    fillMeasure[cnt] = {'beatNum': currBeatNum, 'event': 'off', 'note': 'C', 'velocity': 0,
                                        'midiClk': DrumConstants.SmallestGranularityInTicks, 'octaves': 3,
                                        'pitch': "midi.C_3"}
                    cnt += 1
