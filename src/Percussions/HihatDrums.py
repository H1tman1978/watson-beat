...
def run(BeatInfo, Patterns, tse, rhythmSpeed, pl):
    ...
    HihatDrum[
        'regMeasure'] = collections.OrderedDict()  # this is the normal hihat drum pattern for a measure that is not eos or eop
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            HihatDrum['regMeasure'][patternNum].append(
            ...
            HihatDrum['regMeasure'][patternNum].append(
                {'beatNum': beatNum, 'event': 'off', 'note': DrumConstants.fillNotes['high'][instrument], 'velocity': 0,
                 ...
                 'octave': DrumConstants.fillNoteOctaves['high'][instrument],
                 'pitch': "midi." + DrumConstants.fillNotes['high'][instrument] + "_" + str(
                     DrumConstants.fillNoteOctaves['high'][instrument])})
    ...
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            if (beatNum >= eopStartBeat):  # this beat will be filled in by eopFill
                ...
                HihatDrum['eopMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'on', 'note': DrumConstants.fillNotes['high'][instrument],
                     'velocity': 0, 'midiClk': 0, 'octave': DrumConstants.fillNoteOctaves['high'][instrument],
                     'pitch': "midi." + DrumConstants.fillNotes['high'][instrument] + "_" + str(
                         DrumConstants.fillNoteOctaves['high'][instrument])})
                HihatDrum['eopMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'off', 'note': DrumConstants.fillNotes['high'][instrument],
                     'velocity': 0, 'midiClk': DrumConstants.SmallestGranularityInTicks,
                     'octave': DrumConstants.fillNoteOctaves['high'][instrument],
                     'pitch': "midi." + DrumConstants.fillNotes['high'][instrument] + "_" + str(
                         DrumConstants.fillNoteOctaves['high'][instrument])})
    ...
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            if (beatNum >= eosStartBeat):  # this beat will be filled in by eosFill
                ...
                HihatDrum['eosMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'on', 'note': DrumConstants.fillNotes['high'][instrument],
                     'velocity': 0, 'midiClk': 0, 'octave': DrumConstants.fillNoteOctaves['high'][instrument],
                     'pitch': "midi." + DrumConstants.fillNotes['high'][instrument] + "_" + str(
                         DrumConstants.fillNoteOctaves['high'][instrument])})
