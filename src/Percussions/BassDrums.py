...
def run(BeatInfo, Patterns, tse, rhythmSpeed, pl):
    ...
    BassDrum[
        'regMeasure'] = collections.OrderedDict()  # this is the normal bass drum pattern for a measure that is not eos or eop
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            velocity = random.randint(BeatInfo[beatNum]['bass']['velocityMin'],
                                      BeatInfo[beatNum]['bass']['velocityMax'])
            ...
            BassDrum['regMeasure'][patternNum].append(
                {'beatNum': beatNum, 'event': 'on', 'note': DrumConstants.fillNotes['low']['bass'],
                 'velocity': velocity, 'midiClk': 0, 'octave': DrumConstants.fillNoteOctaves['low']['bass'],
                 'pitch': "midi." + DrumConstants.fillNotes['low']['bass'] + "_" + str(
                     DrumConstants.fillNoteOctaves['low']['bass'])})
            BassDrum['regMeasure'][patternNum].append(
                {'beatNum': beatNum, 'event': 'off', 'note': DrumConstants.fillNotes['low']['bass'], 'velocity': 0,
                 'midiClk': DrumConstants.SmallestGranularityInTicks,
                 'octave': DrumConstants.fillNoteOctaves['low']['bass'],
                 'pitch': "midi." + DrumConstants.fillNotes['low']['bass'] + "_" + str(
                     DrumConstants.fillNoteOctaves['low']['bass'])})
    ...
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            if (beatNum >= eopStartBeat):  # this beat will be filled in by eopFill
                ...
                BassDrum['eopMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'on', 'note': DrumConstants.fillNotes['low']['bass'], 'velocity': 0,
                     'midiClk': 0, 'octave': DrumConstants.fillNoteOctaves['low']['bass'],
                     'pitch': "midi." + DrumConstants.fillNotes['low']['bass'] + "_" + str(
                         DrumConstants.fillNoteOctaves['low']['bass'])})
                BassDrum['eopMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'off', 'note': DrumConstants.fillNotes['low']['bass'], 'velocity': 0,
    ...
    for patternNum in range(numPatterns):
        ...
        for beatNum in range(1, numBeats + 1):
            ...
            if (beatNum >= eosStartBeat):  # this beat will be filled in by eosFill
                ...
                BassDrum['eosMeasure'][patternNum].append(
                    {'beatNum': beatNum, 'event': 'on', 'note': DrumConstants.fillNotes['low']['bass'], 'velocity': 0,
                     'midiClk': 0, 'octave': DrumConstants.fillNoteOctaves['low']['bass'],
                     'pitch': "midi." + DrumConstants.fillNotes['low']['bass'] + "_" + str(
                         DrumConstants.fillNoteOctaves['low']['bass'])})
