...
from . import (BassDrums, CymbalSwell, DrumConstants, DrumFills, DrumKit, HihatDrums, KickDrums, LatinPopDrums,
               MarineraDrumKit, SnareDrums)
...
class Percussions:
    ...
    def run(self):
        ...
        for lyr in self.layerDescs:
            if lyr == 'drumsKick':
                self.percussionLayers['drumsKick'] = KickDrums.run(self.BeatInfo, self.Patterns, self.wbLevers['tse'],
                                                                   self.wbLevers['rhythmSpeed'],
                                                                   self.wbLevers['phraseLength'])
            elif lyr == 'drumsBass':
                self.percussionLayers['drumsBass'] = BassDrums.run(self.BeatInfo, self.Patterns, self.wbLevers['tse'],
                                                                   self.wbLevers['rhythmSpeed'],
                                                                   self.wbLevers['phraseLength'])
            elif lyr == 'drumsSnare':
                self.percussionLayers['drumsSnare'] = SnareDrums.run(self.BeatInfo, self.Patterns, self.wbLevers['tse'],
                                                                     self.wbLevers['rhythmSpeed'],
                                                                     self.wbLevers['phraseLength'])
            elif lyr == 'drumsHihat':
                self.percussionLayers['drumsHihat'] = HihatDrums.run(self.BeatInfo, self.Patterns, self.wbLevers['tse'],
                                                                     self.wbLevers['rhythmSpeed'],
                                                                     self.wbLevers['phraseLength'])
            elif lyr == 'drumsCymbalSwell':
                self.percussionLayers['drumsCymbalSwell'] = CymbalSwell.run(self.BeatInfo, self.wbLevers['tse'],
                                                                            self.wbLevers['rhythmSpeed'],
                                                                            self.wbLevers['phraseLength'])
            elif lyr == 'drumsKit':
                self.percussionLayers['drumsKit'] = DrumKit.run(self.BeatInfo, self.Patterns, self.wbLevers['tse'],
                                                                self.wbLevers['rhythmSpeed'],
                                                                self.wbLevers['phraseLength'])

            elif lyr == 'drumsKitMarinera':
                self.percussionLayers['drumsKitMarinera'] = MarineraDrumKit.run(self.BeatInfo, self.Patterns,
                                                                                self.wbLevers['tse'],
                                                                                self.wbLevers['rhythmSpeed'],
                                                                                self.wbLevers['phraseLength'])
                fills = False
                ...
                self.percussionLayers['drumsLatinPop'] = LatinPopDrums.run(self.BeatInfo, self.Patterns,
                                                                           self.wbLevers['tse'],
                                                                           self.wbLevers['rhythmSpeed'],
                                                                           self.wbLevers['phraseLength'],
                                                                           self.rhyOptions)
        ...
        if (fills):
            ...
            emptyMeasure[1] = {'beatNum': 1, 'event': 'on', 'note': 'C', 'velocity': 1, 'midiClk': 0, 'octave': 0,
                               'pitch': "midi.C_0"}
            emptyMeasure[2] = {'beatNum': 1, 'event': 'off', 'note': 'C', 'velocity': 0, 'midiClk': 1, 'octave': 0,
                               'pitch': "midi.C_0"}

            emptyMeasure[3] = {'beatNum': 1, 'event': 'on', 'note': DrumConstants.fillNotes['low']['kick'],
            ...
            emptyMeasure[4] = {'beatNum': 1, 'event': 'off', 'note': DrumConstants.fillNotes['low']['kick'],
                               'velocity': 0, 'midiClk': numBeats * DrumConstants.SmallestGranularityInTicks - 1,
            ...
            for i in range(self.numPatterns):
                ...
                df = DrumFills.DrumFill(self.wbLevers['tse'], eosStartBeat, totalBeats, self.wbLevers['rhythmSpeed'],
                                        self.wbLevers['complexity'])
                ...
                df = DrumFills.DrumFill(self.wbLevers['tse'], eopStartBeat, totalBeats, self.wbLevers['rhythmSpeed'],
                                        self.wbLevers['complexity'])
