...
class RecordMidi:
    ...
    def __init__(self):
        ...
        self.tsInfo = {'tsNumerator': 4, 'tsDenominator': 4, 'measureLength': 1920, 'resolution': 480, 'format': 0,
                       'bpm': 120}
    ...
    def run(self):
            ...
            with mido.open_input() as port:
                ...
                for message in port:
                    ...
                    if (message.time - self.initialClk >= self.secondsForPhraseLength):
                        print("Initial Clk: ", self.initialClk, "Current Time: ", message.time, self.phraseLength,
                              "measures recorded: ", "Number of seconds recorded: ", self.secondsForPhraseLength,
                              "Stop Recording")
                        endRecording = True
                        break
                    ...
                    if (message.type == 'note_on' or message.type == 'note_off'):
                        if (message.velocity == 0):
                            self.messagesFromMidiInstrument[numNotes] = {'event': 'note_off', 'pitch': message.note,
                                                                         'time': message.time,
                                                                         'velocity': message.velocity}
                        else:
                            self.messagesFromMidiInstrument[numNotes] = {'event': message.type, 'pitch': message.note,
                                                                         'time': message.time,
                                                                         'velocity': message.velocity}
                        numNotes += 1
        ...
        for note in self.messagesFromMidiInstrument:
            self.messagesFromMidiInstrument[note]['time'] = round(
                self.messagesFromMidiInstrument[note]['time'] - self.initialClk, 4)
            self.messagesFromMidiInstrument[note]['starttick'] = round(
                self.messagesFromMidiInstrument[note]['time'] * self.oneSecondInTicks, 0)
    ...
    def removeOverlaps(self):
        ...
        for note in self.midiMessages:
            ...
            for nextNote in range(note + 1, length, 1):
                ...
                actualDuration = self.midiMessages[nextNote]['duration'] * (
                            self.midiMessages[nextNote]['velocity'] / 127.0)
        ...
        for note in self.midiMessages:
            print(note, self.midiMessages[note]['pitch'], self.midiMessages[note]['starttick'],
                  self.midiMessages[note]['endtick'], self.midiMessages[note]['velocity'],
    ...
    def selfQuantizeNotes(self, ticksForQuantization):
        ...
        for note in self.midiMessages:
            ...
            if (mod != 0):  # if note does not start on a quantized note
                ...
                self.midiMessages[note]['endtick'] = int(
                    self.midiMessages[note]['starttick'] + self.midiMessages[note]['duration'])
                self.midiMessages[note]['measure'] = int(
                    self.midiMessages[note]['starttick'] / self.tsInfo['measureLength']) + 1
                self.midiMessages[note]['measureGranularity'] = round(
                    (float(self.midiMessages[note]['starttick']) / self.tsInfo['measureLength']) + 1, 2)

        print()
        for note in self.midiMessages:
            print(note, self.midiMessages[note]['pitch'], self.midiMessages[note]['starttick'],
                  self.midiMessages[note]['endtick'], self.midiMessages[note]['granularity'],
                  self.midiMessages[note]['tie'], self.midiMessages[note]['velocity'],
                  self.midiMessages[note]['duration'], self.midiMessages[note]['measure'],
    ...
    def createMidi(self):
        ...
        for note in self.midiMessages:
            ...
            notes[cnt] = {'event': 'on', 'notestr': notestr, 'octave': octave, 'starttick': starttick,
                          'velocity': velocity, 'pitch': pitch}
            cnt += 1
            notes[cnt] = {'event': 'off', 'notestr': notestr, 'octave': octave, 'starttick': endtick, 'velocity': 0,
                          'pitch': pitch}
            cnt += 1
        ...
        string = "time = midi.TimeSignatureEvent(tick=0, " + "data = [" + str(self.tsInfo['tsNumerator']) + ", " + str(
            tsDenominatorPow) + ", 24, 8])" + "\n";  # 240 bpm
        fout.write(string);
        ...
        for i in notes:
            ...
            if (notes[i]['event'] == 'on'):
                string = "on = midi.NoteOnEvent(tick=" + str(tick) + ", velocity=" + str(
                    velocity) + ", pitch=midi." + pitch + ")\n"
                fout.write(string);
                fout.write("track.append(on)\n")

            else:

                string = "off = midi.NoteOffEvent(tick=" + str(tick) + ", velocity=" + str(
                    velocity) + ", pitch=midi." + pitch + ")\n"
                fout.write(string);
    ...
    def generateStartAndEndTimes(self):
        ...
        for note in self.messagesFromMidiInstrument:
            ...
            for offnote in range(note + 1, length, 1):
                ...
                if (self.messagesFromMidiInstrument[note]['pitch'] == self.messagesFromMidiInstrument[offnote][
                    'pitch']):
                    self.midiMessages[cnt] = {'pitch': self.messagesFromMidiInstrument[note]['pitch'],
                                              ...
                                              'duration': int(self.messagesFromMidiInstrument[offnote]['starttick'] -
                                                              self.messagesFromMidiInstrument[note]['starttick']),
                                              'measure': int(
                                                  self.messagesFromMidiInstrument[note]['starttick'] / self.tsInfo[
                                                      'measureLength']) + 1,
                                              'measureGranularity': round((float(
                                                  self.messagesFromMidiInstrument[note]['starttick']) / self.tsInfo[
                                                                               'measureLength']) + 1, 2),
                                              }
                    cnt += 1
