...
from . import (Constants, MusicTheory)
...
class LayerFills:
    ...
    def getMeasureAndBeatDataForPhrase(self):
        ...
        for item in self.phrase:
            for note in range(len(item['notes'])):
                ...
                if (1):
                    print("Measure: ", mnum, "Beat: ", beatNum, "Note: ", item['notes'][note], 'scale:', item['scale'],
                          'chord:', item['chord'], 'duration:', duration, "startTick: ", startTick, "endTick: ",
                          endTick)

                velocity = item['velocity'][note]
                self.measures['main'][mnum][beatNum] = {'note': item['notes'][note], 'scale': item['scale'],
                                                        'chord': item['chord'], 'duration': duration,
                                                        'velocity': velocity, 'startTick': startTick,
                                                        'endTick': endTick, 'pitch': item['pitches'][note]}
        ...
        for mnum in self.measures['main']:
            ...
            while (quarterBeat > 1):
                ...
                if (quarterBeat in self.measures['main'][mnum] and quarterBeat > 1):
                    mod = self.measures['main'][mnum][quarterBeat]['startTick'] % Constants.NoteDurationDict[
                        'sixteenth']
            ...
            for beatnum in self.measures['main'][mnum]:
                if (beatnum == fillBeatForMeasure):
                    ...
                    alternativeMeasures[mnum].update(
                        self.getMiniMotive(durationLeft, self.measures['main'][mnum][beatnum]['scale'],
                                           self.measures['main'][mnum][beatnum]['chord'], beatnum,
                                           self.measures['main'][mnum][beatnum]['startTick'], prevPitch, homePitch))
                    break
                else:
    ...
    def getMiniMotive(self, duration, scale, chord, beatnum, startTick, prevPitch, homePitch):
        ...
        if (duration <= Constants.NoteDurationDict['quarter']):  # 480
            ...
            NoteDurationList = [Constants.NoteDurationDict['sixteenth'], Constants.NoteDurationDict['eighth'],
                                Constants.NoteDurationDict['threeSixteenth']]
            ...
            NoteDurationList = [Constants.NoteDurationDict['sixteenth'], Constants.NoteDurationDict['eighth'],
                                Constants.NoteDurationDict['threeSixteenth'], Constants.NoteDurationDict['quarter']]

        while True:

            numNotesInMiniMotive = random.choice([2, 3, 3, 3, 4, 4, 4, 5])
            NoteDurationList = [Constants.NoteDurationDict['sixteenth'], Constants.NoteDurationDict['sixteenth'],
                                Constants.NoteDurationDict['sixteenth'], Constants.NoteDurationDict['eighth'],
                                Constants.NoteDurationDict['threeSixteenth'], Constants.NoteDurationDict['sixteenth'],
                                Constants.NoteDurationDict['eighth'], Constants.NoteDurationDict['threeSixteenth'],
        ...
        if (1):
            print("Mini Motive Duration: ", duration, "Scale: ", scale, "chord: ", chord, "Duration Per Motive: ",
                  durationPerMotive, "Num Notes In Motive: ", numNotesInMiniMotive)
        homeOctave = 5
        for item in miniMotive:

            miniMotive[item]['note'] = random.choice(
                MusicTheory.PentatonicScale[scale] + MusicTheory.NotesInScale[scale])
        ...
        while currTicks < duration:
            ...
            for item in miniMotive:
                if (currTicks + miniMotive[item]['duration'] <= duration):
                    altMeasure[beatnum] = {'note': miniMotive[item]['note'], 'scale': scale, 'chord': chord,
                                           'duration': miniMotive[item]['duration'], 'velocity': random.randint(50, 80),
                                           'startTick': startTick, 'endTick': startTick + miniMotive[item]['duration'],
                                           'pitch': miniMotive[item]['pitch']}
