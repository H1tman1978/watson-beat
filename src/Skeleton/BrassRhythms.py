    ...
    def run(self):
        ...
        for mNum in range(numMeasures):
            ...
            if (0):  # and mNum > 0 and muteMeasure ) :
                chosenData = {}
                chosenData = {'notes': ['C'], 'chord': self.homeChord, 'scale': self.homeScale,
                              'duration': [Constants.TSEs[self.tse]['oneMeasure']], 'octaveUp': False, 'velocity': 0}
            ...
            for beat in self.rhythm:
                ...
                for noteNum in range(0, numChordTones):
                    ...
                    chosenData = {'event': 'on', 'notes': [note], 'midiClk': midiClk, 'pitch': midiPitchStr,
                                  'pitches': [pitch], 'chord': self.homeChord, 'scale': self.homeScale,
                                  'duration': duration, 'octaves': [octave], 'velocity': velocity}
                    chosenItems.append(chosenData)
                ...
                for noteNum in range(0, numChordTones):
                    ...
                    chosenData = {'event': 'off', 'notes': [note], 'midiClk': midiClk, 'pitch': midiPitchStr,
                                  'pitches': [pitch], 'chord': self.homeChord, 'scale': self.homeScale,
                                  'duration': duration, 'octaves': [octave], 'velocity': 0}
                    chosenItems.append(chosenData)
