...
class Piano1:
    ...
    def run(self, chordSplitDuration):
        ...
        if (0):
            ...
            print("Home Pitch: ", (MusicTheory.NotesToPitch[chordNotes[0]] + 0) + (self.octave * 12), "Max Pitch: ",
                  self.maxPitch)
        ...
        for item in chordSplitDuration:
            ...
            numNotesInChord = random.choice(list(Constants.NumNotesAndClkInChordBasedOnDuration['staggered'][item].keys()))
            ...
            for note in range(numNotesInChord):
                ...
                eventItems.append({'action': 'StaggeredChordTonesPiano', 'notes': [cNote], 'cNoteIndex': cNoteIndex,
                                   'midiClkStart': midiClkStart, 'midiClkEnd': midiClkEnd, 'pitch': midiPitchStr,
                                   'pitches': [pitch], 'chord': self.homeChord, 'scale': self.homeScale,
                                   'duration': [item], 'octaves': [octave], 'velocity': velocity})
            ...
            for chordSplit in range(len(items)):
                ...
                for item in items[chordSplit]:
                    ...
                    if (cNoteIndex > 2):  # this means this could be a seventh or ninth chord
                        ...
                        if (len(
                                chordNotes) > 3):  # if current chord is a seventh or ninth, give it an option to pick seventh or ninth
                    ...
                    eventItems.append({'event': 'on', 'action': item['action'], 'notes': [cNote],
                                       'midiClkStart': item['midiClkStart'], 'pitch': midiPitchStr,
                                       'pitches': [pitch], 'chord': chord, 'scale': scale, 'duration': item['duration'],
                                       'octaves': [octave], 'velocity': item['velocity']})

                    eventItems.append(
                        {'event': 'off', 'action': item['action'], 'notes': [cNote], 'midiClkStart': item['midiClkEnd'],
                         'pitch': midiPitchStr,
                         'pitches': [pitch], 'chord': chord, 'scale': scale, 'duration': item['duration'],
                         'octaves': [octave], 'velocity': 0})
                ...
                if (len(eventItems) > 0):
                    ...
                    for val in range(1, len(sortedEventItems), 1):
                        sortedEventItems[val]['midiClk'] = sortedEventItems[val]['midiClkStart'] - \
                                                           sortedEventItems[val - 1]['midiClkStart']
    ...
    def getSimpleChordRhythms(self, newItems):
        ...
        for chordSplit in range(len(newItems)):
            eventItems = []
            for item in newItems[chordSplit]:
                if ((item['midiClkStart'] % Constants.NoteDurationDict['eighth'] == 0) and (
                        item['midiClkEnd'] - item['midiClkStart'] >= Constants.NoteDurationDict[
                    'eighth'] - 50)):  # exclude all 16th notes
