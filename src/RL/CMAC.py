...
class CMAC:
    ...
    def __init__(self, alpha, numFloatInputs, numIntInputs, memorySize, gamma, numTilings, tableDimensionality):
        ...
        for i in range(memorySize):
            # self.u.append ( 1.0 / ( 1.0 - gamma ) ) self.u.append ( 1.0 / ( 1.0 - gamma - 0.01 ) ) # this is to
            # account for the fact that gamma = 1 for episodic tasks, and gamma < 1 for continuing tasks.
            # self.u.append ( random.randint ( 1, 20 ) ) #1.0 / ( 1.0 - gamma - 0.01 ) ) # this is to account for the
            # fact that gamma = 1 for episodic tasks, and gamma < 1 for continuing tasks.
            self.u.append(2)
            # 1.0 / ( 1.0 - gamma - 0.01 ) ) # this is to account for the fact that gamma = 1 for episodic tasks,
            # and gamma < 1 for continuing tasks.
            # print ( "Memory Size: ", memorySize, "self.u[", i, "]", self.u[i],
            # "Gamma:" , gamma, "1.0-gamma", 1.0 - gamma )
    ...
    def predict(self):
        ...
        self.tiles = self.Tile.GetTiles(self.tiles, self.numTilings, self.memorySize, self.floatInputs,
                                        self.numFloatInputs, self.intInputs, self.numIntInputs)
    ...
    def printCMACParams(self):
        ...
        print("Float Inputs; ", self.floatInputs, self.numFloatInputs, "Int Inputs; ", self.intInputs,
              self.numIntInputs, "Size of CMAC Arrays (memory size) : ", self.memorySize, "Tiles: ", self.tiles,
              self.numTilings)
        print()
        return
