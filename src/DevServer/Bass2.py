...
class Bass2:
    ...
    def run(self):
        ...
        for noteId in range(len(self.rhythm)):
            ...
            chosenData = {'action': chosenAction, 'notes': chosenNote, 'chord': self.homeChord, 'scale': self.homeScale,
                          'duration': [chosenDuration], 'octaveUp': octaveUp}
        ...
        for trIter in range(trainingIterations):  # called for every training iteration
            ...
            for noteId in range(len(self.rhythm)):
                ...
                if (noteId == 0):  # this is the first note to be played, so choose between home note and the octave up
                    ...
                    chosenNote, octaveUp = self.getNextNoteToBePlayed(chosenAction)
                    chosenReward = 0
                    chosenData = {'action': chosenAction, 'notes': chosenNote, 'chord': self.homeChord,
                                  'scale': self.homeScale, 'duration': [chosenDuration], 'octaveUp': octaveUp}
                else:
                    ...
                    if (randForExploration >= exploitationThreshold):
                        ...
                        for action in range(self.numActions):
                            ...
                            if (initial or q >= qMax):
                                qMax = q
                                initial = False
                                chosenData = {'action': currentAction, 'notes': currentNote, 'chord': self.homeChord,
                                              'scale': self.homeScale, 'duration': [currentDuration],
                                              'octaveUp': octaveUp}
