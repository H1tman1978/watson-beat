...
class Bass3:
    ...
    def run(self):
        ...
        if (0):
            print("self.duration: :", self.duration, "Num Beats: ", numBeats, "Num Measures: ", numMeasures,
                  "halfMeasurebeat: ", halfMeasureBeat, self.rhythm)
        ...
        for mNum in range(numMeasures):
            ...
            if (mNum > 0 and muteMeasure):
                chosenData = {}
                chosenData = {'action': 'PLAY_HOME_NOTE', 'notes': ['C'], 'chord': self.homeChord,
                              'scale': self.homeScale, 'duration': [Constants.TSEs[self.tse]['oneMeasure']],
                              'octaveUp': False, 'velocity': 0}
            ...
            for beat in self.rhythm:
                ...
                if (beat >= halfMeasureBeat and muteHalfMeasure):
                    ...
                    chosenData = {'action': 'PLAY_HOME_NOTE', 'notes': ['C'], 'chord': self.homeChord,
                                  'scale': self.homeScale, 'duration': [chosenDuration], 'octaveUp': False,
                                  'velocity': 0}
                    chosenItems.append(chosenData)
                    break
                ...
                if (beat == 0):  # this is the first note to be played, so choose between home note and the octave up
                    ...
                    chosenAction = self.Actions[
                        random.randint(0, totalActions)]  # not the first note. Choose any action
...
def getRhyDurations(rhyOption, tse):
    ...
    for beatIndex, beat in enumerate(rhyOption):
        ...
        if (chosenDuration >= Constants.NoteDurationDict['eighth'] and chosenDuration < Constants.NoteDurationDict[
            'quarter']):
            ...
            if (random.randint(0, 100) > 70):  # split into 16th note + Rest, or 8th note + Rest or quarter note + rest
                duration = random.choice(
                    [(Constants.NoteDurationDict['sixteenth'], 1), (Constants.NoteDurationDict['eighth'], 2),
                     (Constants.NoteDurationDict['quarter'], 4)])
