...
class ChordProgressionJumps:
    ...
    def CreatePenaltyForChordProgressionJumps(self, homeScale):
        ...
        for currChord in MusicTheory.AllChords:
            ...
            pullToHome = [MusicTheory.pitchToNotes[(homeIndex + 1) % 12],
                          MusicTheory.pitchToNotes[(homeIndex + 11) % 12]]

            note1Index = MusicTheory.NotesToPitch[currChordNotes[1]]
            pullToNote1 = [MusicTheory.pitchToNotes[(note1Index + 1) % 12],
                           MusicTheory.pitchToNotes[(note1Index + 11) % 12]]

            note2Index = MusicTheory.NotesToPitch[currChordNotes[2]]
            pullToNote2 = [MusicTheory.pitchToNotes[(note2Index + 1) % 12],
                           MusicTheory.pitchToNotes[(note2Index + 11) % 12]]
            ...
            if (len(currChordNotes) >= 4):
                ...
                pullToNote3 = [MusicTheory.pitchToNotes[(note3Index + 1) % 12],
                               MusicTheory.pitchToNotes[(note3Index + 11) % 12]]

            if (len(currChordNotes) >= 5):
                NinthChord = True
                note4Index = MusicTheory.NotesToPitch[currChordNotes[4]]
                pullToNote4 = [MusicTheory.pitchToNotes[(note4Index + 1) % 12],
                               MusicTheory.pitchToNotes[(note4Index + 11) % 12]]

            self.ChordProgressions[currChord] = collections.OrderedDict()

            self.ChordProgressions[currChord]['obvious'] = {'PLAY_CHORD_PROGRESSION_TONE_SML_JUMP': {},
                                                            'PLAY_CHORD_PROGRESSION_TONE_MID_JUMP': {},
                                                            'PLAY_CHORD_PROGRESSION_TONE_BIG_JUMP': {}}
            self.ChordProgressions[currChord]['unusual'] = {'PLAY_CHORD_PROGRESSION_TONE_SML_JUMP': {},
                                                            'PLAY_CHORD_PROGRESSION_TONE_MID_JUMP': {},
                                                            'PLAY_CHORD_PROGRESSION_TONE_BIG_JUMP': {}}
            self.ChordProgressions[currChord]['obscure'] = {'PLAY_CHORD_PROGRESSION_TONE_SML_JUMP': {},
                                                            'PLAY_CHORD_PROGRESSION_TONE_MID_JUMP': {},
                                                            'PLAY_CHORD_PROGRESSION_TONE_BIG_JUMP': {}}
            ...
            for nextChord in MusicTheory.AllChords:
                ...
                if (0 and (currChord == 'CMajor' or currChord == 'EMinor')):
                    print("CurrChord: ", currChord, "CurrScale: ", currScale, "NextChord: ", nextChord, "NextScale: ",
                          nextScale, "Penalty: ", totalPenalty)
                    # print ( totalPenalty )
                ...
                if (totalPenalty >= -4):
                    ...
                    if (totalPenalty >= 0):
                        self.ChordProgressions[currChord]['obvious']['PLAY_CHORD_PROGRESSION_TONE_SML_JUMP'][
                            nextChord] = (
                        nextChord, nextScale, numCommonNotesBetweenChords, numNotCommonNotesBetweenScalePenalty,
                        pullNotesPenalty, totalPenalty)
                    elif (totalPenalty >= -2 and totalPenalty <= 0):
                        self.ChordProgressions[currChord]['obvious']['PLAY_CHORD_PROGRESSION_TONE_MID_JUMP'][
                            nextChord] = (
                        nextChord, nextScale, numCommonNotesBetweenChords, numNotCommonNotesBetweenScalePenalty,
                        pullNotesPenalty, totalPenalty)
                    elif (totalPenalty >= -4 and totalPenalty <= -3):
                        self.ChordProgressions[currChord]['obvious']['PLAY_CHORD_PROGRESSION_TONE_BIG_JUMP'][
                            nextChord] = (
                        nextChord, nextScale, numCommonNotesBetweenChords, numNotCommonNotesBetweenScalePenalty,
                        pullNotesPenalty, totalPenalty)
                    ...
                    if (totalPenalty >= -8 and totalPenalty <= -5):
                        ...
                        self.ChordProgressions[currChord]['unusual']['PLAY_CHORD_PROGRESSION_TONE_MID_JUMP'][
                            nextChord] = (
                        nextChord, nextScale, numCommonNotesBetweenChords, numNotCommonNotesBetweenScalePenalty,
                        pullNotesPenalty, totalPenalty)
                    elif (totalPenalty >= -15 and totalPenalty <= -13):
                        self.ChordProgressions[currChord]['unusual']['PLAY_CHORD_PROGRESSION_TONE_BIG_JUMP'][
                            nextChord] = (
                        nextChord, nextScale, numCommonNotesBetweenChords, numNotCommonNotesBetweenScalePenalty,
                        pullNotesPenalty, totalPenalty)
                    ...
                    if (totalPenalty >= -18 and totalPenalty <= -16):
                        ...
                        self.ChordProgressions[currChord]['obscure']['PLAY_CHORD_PROGRESSION_TONE_MID_JUMP'][
                            nextChord] = (
                        nextChord, nextScale, numCommonNotesBetweenChords, numNotCommonNotesBetweenScalePenalty,
                        pullNotesPenalty, totalPenalty)
                    elif (totalPenalty >= -24 and totalPenalty <= -22):
                        self.ChordProgressions[currChord]['obscure']['PLAY_CHORD_PROGRESSION_TONE_BIG_JUMP'][
                            nextChord] = (
                        nextChord, nextScale, numCommonNotesBetweenChords, numNotCommonNotesBetweenScalePenalty,
                        pullNotesPenalty, totalPenalty)
