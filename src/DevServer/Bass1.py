...
from Skeleton import (Constants, MusicTheory, Environment)
...
class Bass1:
    ...
    def __init__(self, wbLevers):
        ...
        self.penaltyBase = (
                                       self.trueMaxPenalty * -1) + 1  # why 25, beacuse the adder for making all penalties positive is 25. why 25, because the maximum penalty is -24 and the min penalty = 4
        ...
        self.penaltyBase = (
                                       self.trueMaxPenalty * -1) + 1  # why 20, beacuse the adder for making all penalties positive is 20. why 20, because the maximum penalty is -19 and the min penalty = 7
        ...
        self.Knobs, self.Actions, self.ReverseActions = ComplexityKnobs.setKnobs('bass1', wbLevers['complexity'],
                                                                                 layerParams)
        self.numActions = len(self.Actions)
        self.defaultAction = 3  # 'PLAY_CHORD_PROGRESSION_TONE_FOR_PRIMARY_KEY'

        # Create the bass1 environment        
        self.env = Environment.Bass1Environment(wbLevers['tse'], wbLevers['phraseLength'], wbLevers['primaryScale'],
                                                wbLevers['mood'])
    ...
    def initializeQLearningObject(self, rlParams):
        ...
        self.qLearn = QLearning.QLearning(self.numFeatures, self.features, self.numActions, 1024, 0.50, 16, 2, 0.95,
                                          0.10, 1.0, self)
    ...
    def assignCreditOrBlameForAction(self, chosenAction, data):  # data holds the chord and the scale
        ...
        if (0):
            print("Penalty: ", penalty, "cprogjumplow: ", self.Knobs['cprogJumpLow'], "cprogjumphigh: ",
                  self.Knobs['cprogJumpHigh'], "Immediate Reward: ", reward)
    ...
    def getNextState(self, action, data):  # data will include the following: chord, scale,
        ...
        if (homeChordMovementDistance > 1):
            print("homeChordMovementDistance : ", homeChordMovementDistance, "Penalty: ", penalty, "penalty min: ",
                  self.penaltyMin)
    ...
    def getInitialChordForScale(self, scale):
        ...
        penalty = \
        self.env.cpJumps.ChordProgressions[homeChord]['obvious']['PLAY_CHORD_PROGRESSION_TONE_SML_JUMP'][homeChord][5]
    ...
    def getCPJump(self, action):
        ...
        if (0):
            print()
            print("Home Scale: ", homeScale, "Current Chord: ", currChord, "cpComplexity: ", cpComplexity, "Action: ",
                  action)
        ...
        if (len(allTuples) == 0):
            ...
            for i in range(
                    10):  # try for 10 times to get a chord that fits into the max number of scales. if not use previous chord as the option

                nextChord = random.choice(list(allTuples.keys()))
                ...
                if (0):
                    print("i: ", i, "Home Scale: ", homeScale, "Current Chord: ", currChord, "cpComplexity: ",
                          cpComplexity, "Action: ", action)
            ...
            nextHomeNote = MusicTheory.AllChords[nextChord][0]
            penalty = self.env.cpJumps.ChordProgressions[currChord][cpComplexity][action][nextChord][
                          5] + self.penaltyBase
            if (penalty < 1):  # sanity check
                print("Penalty is less than 1. That can never happen. Need to increase adder");
                print("Abort")
                print("Next Chord: ", nextChord, "Next Scale: ", nextScale, 'homeNote: ', nextHomeNote, "Penalty: ",
                      penalty)
                print()
                sys.exit(0)
    ...
    def run(self, CPTicks):  # CPTicks stores the chord durations for each chord
        ...
        for trIter in range(trainingIterations):  # called for every training iteration
            ...
            for cpIndex in range(len(CPTicks)):  # called everytime you want to perform an action
                ...
                if (
                        cpIndex == 0):  # this is the first chord to be played, so choose 'PLAY_CHORD_PROGRESSION_TONE_FOR_PRIMARY_KEY'
                    ...
                    chosenReward = 0
                    numInitialActions += 1
                    chosenData = {'action': chosenAction, 'notes': [chosenHomeNote], 'chord': chosenChord,
                                  'scale': chosenScale, 'duration': [chosenDuration], 'penalty': penalty}
                    self.scalesInUse[chosenScale] = True
                    if (0):
                        print("Chord: ", cpIndex + 1, "Action type: ", typeOfAction, "Action: ", chosenAction,
                              chosenChord)
                    ...
                    if (0):
                        ratioIter = round(float(trIter * 1.0 / trainingIterations * 1.0), 2)
                        print("trIter: ", trIter, "Num Iterations: ", trainingIterations, "Ratio: ", ratioIter,
                              "Rand Exploitation: ", exploitationThreshold, "Rand Exploration: ", randForExploration)
                    ...
                    if (randForExploration >= exploitationThreshold):
                        ...
                        for action in range(self.numActions):
                            ...
                            currentHomeNote, currentChord, currentScale, penalty = self.getNextChordToBePlayed(
                                currentAction)

                            self.getNextState(currentAction,
                                              {'chord': currentChord, 'scale': currentScale, 'penalty': penalty})
                            q = self.qLearn.predictQFactor(self.nextState, self.ReverseActions[currentAction])

                            if (0):
                                print("Chord: ", cpIndex + 1, "Action type: ", typeOfAction, "Action: ", currentAction,
                                      currentChord)
                                print("i: ", action, "Notes: ", currentHomeNote, "currentChord: ", currentChord,
                                      "currentScale: ", currentScale, "Duration: ", currentDuration, "Action: ",
                                      currentAction, "Next State: ", self.nextState.stats, "penalty: ", penalty)
                                print("i: ", action, "q:", q, "qMax: ", qMax)

                            if (initial or q >= qMax):
                                qMax = q
                                initial = False
                                chosenData = {'action': currentAction, 'notes': [currentHomeNote],
                                              'chord': currentChord, 'scale': currentScale,
                                              'duration': [currentDuration], 'penalty': penalty}
                                # end for action in range(self.numActions):
                        if (0):
                            print("Chord: ", cpIndex + 1, "Action type: ", typeOfAction, "Action: ",
                                  chosenData['action'], chosenData['chord'])
