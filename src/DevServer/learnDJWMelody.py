...
class RBM:
    ...
    def __init__(self, num_visible, num_hidden, learning_rate=0.1):
        ...
        self.weights = 0.1 * np.random.randn(int(self.num_visible), int(self.num_hidden))
    ...
    """
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            pos_hidden_states = pos_hidden_probs > np.random.rand(num_examples, int(self.num_hidden) + 1)
    ...
    """
        ...
        # sampled from a training example.
        visible_states = np.ones((num_examples, int(self.num_visible) + 1))
    ...
    for i in range(num_time_slices):
        ...
        for j in range(0, num_note_slices, 2):
            if (j == 0):
                ...
                if (biasToTieNeuron):
                    training_data[cnt + 1] = random.choice(
                        [0.00, 0.15, 0.0, 0.15, 0.15])  # choose tie note 60% of the time
                else:
...
def createMidi(pitch, startTime, endTime, foutName):
    ...
    for i in range(len(pitch)):
        ...
        events[cnt] = {'event': 'on', 'midiClk': clk, 'note': note, 'pitch': "midi." + noteStr + "_" + str(octave),
                       'octave': octave, 'velocity': random.randint(50, 80)}
        cnt += 1
        events[cnt] = {'event': 'off', 'midiClk': duration, 'note': note,
                       'pitch': "midi." + noteStr + "_" + str(octave), 'octave': octave, 'velocity': 0}
        cnt += 1
