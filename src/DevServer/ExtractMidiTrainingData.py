...
class MidiTrainingData:
    ...
    def removeOverlaps(self):
        ...
        trackName = 'melody'
        trackName = 'other'
        lastItem = list(self.Tracks[trackName][1].keys())[-1]
                ...
                for nextItem in range(item + 1, lastItem, 1):
                    ...
                    if nextItem in self.Tracks[trackName][1]:
                        ...
                        if (s1 < e2 and ((e1 - s2) > 100)):
                            print("Overlap found between: ", item, nextItem, self.Tracks[trackName][1][item],
                                  self.Tracks[trackName][1][nextItem])
                            print()
    ...
    def ExtractChords(self, Chords, Scales, durations):
        ...
        while (chord != len(Chords)):
            for item in self.Tracks[trackName][1]:
                ...
                if (0):
                    print("Item: ", self.Tracks[trackName][1][item], "Chord: ", chord, "End Chord Duration: ",
                          currDuration)
                ...
                if (self.Tracks[trackName][1][item]['Clk'][1] > currDuration and self.Tracks[trackName][1][item]['Clk'][
                    0] < currDuration):
                    ...
                    newItemB['Clk'][
                        0] = currDuration  # change beginning of note B to start of new chord ( or end of current chord)
                    ...
                    self.Tracks[trackName][1][item]['duration'] = self.Tracks[trackName][1][item]['Clk'][1] - \
                                                                  self.Tracks[trackName][1][item]['Clk'][0]
        ...
        if (self.melodyPL[numMelNotes - 1]['Clk'][1] != currDuration):
            ...
            self.melodyPL[numMelNotes]['duration'] = self.melodyPL[numMelNotes]['Clk'][1] - \
                                                     self.melodyPL[numMelNotes]['Clk'][0]
            if (chord >= len(durations)):
    ...
    def readDumpFile(self, midiDumpFile):
        ...
        for line in midiFin:
            ...
            if line.startswith("midi.Pattern"):
                ...
                for i in range(len(split_line)):
                    if (split_line[i] == 'resolution'):
                        ...
                        maxTicks = ((
                                        self.resolution) / 2) * 13 * 8  # self.resolution/2 gives the ticks for an 8th note. multiplying by 13 gives the number of ticks for one measure when using the longest time signature. multiplying that by 8 gives us 8 measures worth of ticks.
                        break
            ...
            if re.search("midi.TrackNameEvent", line, re.IGNORECASE):
                ...
                for i in range(len(split_line)):
                    if (split_line[i] == ' text'):
                        t = split_line[i + 2]
                        if ((re.search('vocal', t, re.IGNORECASE) and not re.search('back', t, re.IGNORECASE)) or (
                        re.search('melody', t, re.IGNORECASE))):
                            trackName = 'melody'
            ...
            if ((re.search("midi.NoteOnEvent", line)) or (re.search("midi.NoteOffEvent", line))):
                ...
                if (pitch in track1):
                    lastEvent = list(track1[pitch].keys())[-1]
                    ...
                    if (numEvents % 2 == 0 and event == 'on'):
                        ...
                        if (int(glbClk * self.multiplier) - track1[pitch][lastEvent][-1] != 0):
                            ...
                            track2[lastEvent] = {'pitch': pitch, 'Clk': track1[pitch][lastEvent],
