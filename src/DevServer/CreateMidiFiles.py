...
class Midi:
    ...
    def ReplicateLayers(self):
        ...
        for layer in self.uniqLayers[
            0]:  # the 0 indicates the first unique melody. the index 0 is needed to capture the differnt layers that have been generates ( i.e.: Bass1, Bass2, Melody, Pentatonic Melody )
        ...
        if (layer == 'drumsCymbalSwell'):
            for phrase in range(repCount):
                ...
                for mnum in range(pl):
                    if (
                            phrase == repCount - 1 and mnum >= pl - 1):  # if it is the last phrase in the rep count and the second last measure, then cymbal swell should start
                        continue
                    midiData[phrase][mnum] = self.uniqLayers[melId][layer][
                        pl]  # all other phrases and measures are empty
                if (phrase == repCount - 1):  # last phrase in rep count
                    midiData[phrase][pl - 1] = self.uniqLayers[melId][layer][
                        pl - 1]  # cymbal swell starts at second last measure
        else:
            for phrase in range(repCount):
                midiData[phrase] = collections.OrderedDict()
                if (phrase == repCount - 1):  # last phrase in rep count
    ...
    def CreateMidiForLayers(self):
        ...
        emptyEvents = [{'event': 'on', 'midiClk': 0, 'pitch': 'midi.C_0',
                        'velocity': 1}]  # needed for same reason as muted first section
            ...
            for sec in self.MidiData[layer]:
                ...
                if (layer == 'mel5'):
                    ...
                    for phrase in self.MidiData[layer][sec]:
                        ...
                        if (1):
                            print("Melody Section: ", sec, " Phrase : ", phrase, " starts at global clock: ", glbClk,
                                  " Section muted: ", mute)

                            # begin new phrase event
                        phrStr = "# Phrase: " + str(phrase)
                        phrEvent = [{'event': 'comment', 'data': phrStr}]
                        phrEvents, currClk, maxPitch, minPitch = self.CreateMidiEventsForMelody(
                            self.MidiData[layer][sec][phrase], glbClk, mute, firstSectionMuted)
                        ...
                        foutName = "WB_Mvmt" + str(self.movement['id']) + "_" + layer + "_Sec" + str(
                            sec) + "_Phrase" + str(phrase)
                        ...
                        if (1):
                            debugPhrEvent, deugCnt, maxPitch, minPitch = self.CreateMidiEventsForMelody(
                                self.MidiData[layer][sec][phrase], 0, mute, firstSectionMuted)
                            self.MidiEvents[layer] += phrEvent + debugPhrEvent

                elif (layer == 'piano1' or layer == 'rhythmChords' or layer == 'strings1' or layer.startswith(
                        'strings2')):
                    self.MidiEvents[layer] += self.CreateMidiEventsForRhythm(self.MidiData[layer][sec], mute,
                                                                             firstSectionMuted)

                elif (layer.startswith('drums')):
                    self.MidiEvents[layer] += self.CreateMidiEventsForPercussions(self.MidiData[layer][sec], mute,
                                                                                  firstSectionMuted)
                    if (layer == 'drumsKick'):
                        if ('fillsForDrums' not in self.MidiEvents):
                            self.MidiEvents['fillsForDrums'] = []
                        self.MidiEvents['fillsForDrums'] += self.CreateMidiEventsForPercussionFills(
                            self.MidiData['fillsForDrums'][sec], mute, firstSectionMuted)

                elif (not layer.startswith('fillsForDrums')):
                    self.MidiEvents[layer] += self.CreateMidiEventsForOtherLayers(self.MidiData[layer][sec], mute,
                                                                                  firstSectionMuted, octave)
    ...
    def CreateMidiEventsForMelody(self, midiData, glbClk, mute, firstSectionMuted):
        ...
        if (glbClk != 0):
            localClk = glbClk - 1
            midiEvents.append({'event': 'on', 'midiClk': 0, 'pitch': 'midi.C_0',
                               'velocity': 1})  # needed for same reason as muted first section
    ...
    def WriteMidiEvents(self, fout, data):
        for item in data:
            ...
            if (item['event'] == 'on'):
                string = "on = midi.NoteOnEvent(tick=" + str(item['midiClk']) + ", velocity=" + str(
                    item['velocity']) + ", pitch=" + item['pitch'] + ")" + "\n";
                fout.write(string);
                fout.write("track.append(on)\n")
            elif (item['event'] == 'off'):
                string = "off = midi.NoteOffEvent(tick=" + str(item['midiClk']) + ", velocity=" + str(
                    item['velocity']) + ", pitch=" + item['pitch'] + ")" + "\n";
                fout.write(string);
                fout.write("track.append(off)\n")
            elif (item['event'] == 'tse'):
                string = "#time = midi.TimeSignatureEvent(tick=" + str(item['midiClk']) + ", data = " + item[
                    'data'] + ")" + "\n";
                fout.write(string);
                fout.write("#track.append(time)\n")
            elif (item['event'] == 'tempo'):
                string = "#tempo = midi.SetTempoEvent(tick=" + str(item['midiClk']) + ", data = " + item[
                    'data'] + ")" + "\n";
                fout.write(string);
    ...
    def WriteInitialMidiFileSequence(self, fout):
        ...
        fout.write("import python3-midi as midi\n");
        ...
        for phrase in midiData:
            ...
            for mnum in midiData[phrase]:
                ...
                for item in midiData[phrase][mnum]:
                    ...
                    midiEvents.append({'event': item['event'], 'midiClk': item['midiClk'], 'pitch': item['pitch'],
                                       'velocity': velocity})
