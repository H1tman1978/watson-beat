...
def createMidiEvents(movement, uniqLayers, layerTemplateForSection):
    ...
    for layer in uniqLayers[0]:
        # the 0 indicates the first unique melody. the index 0 is needed to capture the different layers that
        # have been generates ( i.e.: Bass1, Bass2, Melody, Pentatonic Melody )
        if (layer == 'mel5'):
        ...
        for sec in MidiData[layer]:
            ...
            if (layer == 'mel5'):
                ...
                for phrase in MidiData[layer][sec]:
                    ...
                    phrEvents, currClk = CreateMidiEventsForMelody(MidiData[layer][sec][phrase], glbClk, mute,
                                                                   firstSectionMuted)
                    ...
                    foutName = "WB_Mvmt" + str(movement['id']) + "_" + layer + "_Sec" + str(sec) + "_Phrase" + str(
                        phrase)
                    ...
                    if (1):
                        debugPhrEvent, deugCnt = CreateMidiEventsForMelody(MidiData[layer][sec][phrase], 0, mute,
                                                                           firstSectionMuted)
                ...
                MidiEvents[layer] += secEvent + tempoEvent + tseEvent
                MidiEvents[layer] += CreateMidiEventsForOtherLayers(MidiData[layer][sec], mute, firstSectionMuted,
                                                                    octave)
    ...
    for phrase in midiData:
        ...
        for chordSplit in midiData[phrase]:
            for item in chordSplit:
                ...
                midiEvents.append(
                    {'event': item['event'], 'midiClk': item['midiClk'], 'pitch': item['pitch'], 'velocity': velocity})
