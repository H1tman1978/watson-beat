...
from . import (Bass1, ExtractMidiTrainingData, LayerFills, MusicTheory, Rhythm, learnDJWMelody)
...
class Section:
    ...
    def run(self):
        ...
        for chordId in range(self.numChords):
            self.chordDict[chordId] = {'duration': self.bass1Obj.Chords[phraseNum][chordId][0]['duration'][0],
    ...
    def initializeAndRunDJWatsonMelody(self):
        ...
        for chId in midiTrainingData.melody:
            ...
            for item in range(len(new_pitch)):
                ...
                itemsForOctaveBalancing.append({'chordId': chId, 'chord': tr_chord[item], 'scale': tr_scale[item],
                                                'duration': [tr_endTime[item] - tr_startTime[item]],
                                                'velocity': [random.randint(50, 80)], 'pitches': [new_pitch[item]],
                                                'notes': [note], 'Clk': [tr_startTime[item], tr_endTime[item]]})

                continue

                if (item + 1 < len(new_pitch) and tr_endTime[item] < tr_startTime[item + 1]):
                    self.mel5Data[phraseNum][chId].append(
                        {'nullNote': True, 'chord': tr_chord[item], 'scale': tr_scale[item],
                         'duration': [tr_startTime[item + 1] - tr_endTime[item]], 'velocity': [0],
                         'pitches': [new_pitch[item]], 'notes': [note],
                         'Clk': [tr_endTime[item], tr_startTime[item + 1]]})
                    itemsForOctaveBalancing.append({'nullNote': True, 'chord': tr_chord[item], 'scale': tr_scale[item],
                                                    'duration': [tr_startTime[item + 1] - tr_endTime[item]],
                                                    'velocity': [0], 'pitches': [new_pitch[item]], 'notes': [note],
                                                    'Clk': [tr_endTime[item], tr_startTime[item + 1]]})
                    # print ( "I am here 1" )
                    ...
                    self.mel5Data[phraseNum][chId].append(
                        {'nullNote': True, 'chord': tr_chord[item], 'scale': tr_scale[item],
                         'duration': [total - tr_endTime[item]], 'velocity': [0], 'pitches': [new_pitch[item]],
                         'notes': [note], 'Clk': [tr_endTime[item], total]})
                    itemsForOctaveBalancing.append({'nullNote': True, 'chord': tr_chord[item], 'scale': tr_scale[item],
                                                    'duration': [total - tr_endTime[item]], 'velocity': [0],
                                                    'pitches': [new_pitch[item]], 'notes': [note],
                                                    'Clk': [tr_endTime[item], total]})
            ...
            if (firstItem):
                ...
                if (currPitch > prevPitch):
                    ...
                    if (cumulativeStep + currStep > 10 or currStep > 8):
                        if (0):
                            print("Here : 1a cumSt + cstep > 10 or cstep >= 8, cumulativeStep : ", cumulativeStep,
                                  " currStep: ", currStep)
                        ...
                        if (not itemForBalancingFound):  # no item found for balancing. make this a rest note
                            ...
                            if (0):
                                print("Here : 1b, no pitch found for balancing, itemCnt: ", itemCnt, "item: ",
                                      itemsForOctaveBalancing[itemCnt])
                            itemCnt += 1;
                            continue
                        else:
                            if (0):
                                print("Here : 1c Item Found for balancing: ", itemToCheck + 1, "Before balancing: ",
                                      itemsForOctaveBalancing[itemToCheck + 1], "diff: ")
                            cntUp = 0
                            while (cntUp < 2):
                                itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] -= 12
                                cntUp += 1
                                if (abs(itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] -
                                        itemsForOctaveBalancing[itemToCheck]['pitches'][0]) <= 10):
                                    break
                            if (0):
                                print("Here : 1c Item Found for balancing: ", itemToCheck + 1, "After balancing: ",
                                      itemsForOctaveBalancing[itemToCheck + 1])

                            if (itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] < minPitch):
                                itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] += 12
                                if (0):
                                    print("Here : 1d Item less than min pitch. Increase by octave: ", itemToCheck + 1,
                                          "After Increasing by octave: ", itemsForOctaveBalancing[itemToCheck + 1])

                            cumulativeStep = itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] - \
                                             itemsForOctaveBalancing[0]['pitches'][0]
                        ...
                        if (0):
                            print("Here : 1e cumSt + cstep <= 10 and cstep < 8, cumulativeStep : ", cumulativeStep,
                                  " currStep: ", currStep)
                    ...
                    if (cumulativeStep + currStep < -10 or currStep < -8):
                        if (0):
                            print("Here : 2a cumSt + cstep < -10 or cstep <= -8, cumulativeStep : ", cumulativeStep,
                                  " currStep: ", currStep)
                        ...
                        if (not itemForBalancingFound):  # no item found for balancing. make this a rest note
                            if (0):
                                print("Here : 2b, no pitch found for balancing, itemCnt: ", itemCnt, "item: ",
                                      itemsForOctaveBalancing[itemCnt])
                            itemsForOctaveBalancing[itemCnt]['velocity'][0] = 0
                            itemCnt += 1;
                            continue
                            ...
                            if (0):
                                print("Here : 2c Item Found for balancing: ", itemToCheck + 1, "Before balancing: ",
                                      itemsForOctaveBalancing[itemToCheck + 1])
                            cntUp = 0
                            while (cntUp < 2):
                                itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] += 12
                            ...
                            if (0):
                                print("Here : 2c Item Found for balancing: ", itemToCheck + 1, "After balancing: ",
                                      itemsForOctaveBalancing[itemToCheck + 1])

                            if (itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] > maxPitch):
                                itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] -= 12
                                if (0):
                                    print("Here : 2d Item greater than man pitch. Decrease by octave: ",
                                          itemToCheck + 1, "After Decreasing by octave: ",
                                          itemsForOctaveBalancing[itemToCheck + 1])
                            prevPitch = itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0]
                            cumulativeStep = itemsForOctaveBalancing[itemToCheck + 1]['pitches'][0] - \
                                             itemsForOctaveBalancing[0]['pitches'][0]
                        ...
                        if (0):
                            print("Here : 2e cumSt + cstep >= 10 and cstep > -8, cumulativeStep : ", cumulativeStep,
                                  " currStep: ", currStep)
                        cumulativeStep += currStep
                        prevPitch = currPitch
                        itemCnt += 1
        ...
        if (0):
            ...
            for itemIndex, item in enumerate(itemsForOctaveBalancing):
                if (itemIndex > 0):
                    print(itemIndex, "Pitch: ", item['pitches'][0], "diff: ", abs(
                        itemsForOctaveBalancing[itemIndex - 1]['pitches'][0] -
                        itemsForOctaveBalancing[itemIndex]['pitches'][0]), item['duration'][0], item['notes'][0])
                else:
        ...
        for item in range(len(itemsForOctaveBalancing)):
            ...
            if (itemsForOctaveBalancing[item]['Clk'][1] < itemsForOctaveBalancing[item + 1]['Clk'][0]):
                ...
                newItem['duration'] = [
                    itemsForOctaveBalancing[item + 1]['Clk'][0] - itemsForOctaveBalancing[item]['Clk'][1]]
                newItem['velocity'] = [0]
                self.mel5Data[phraseNum][chId].append(newItem)
    ...
    def fixOctaves(self, itemsForOctaveBalancing):
        ...
        if (0):
            ...
            for itemIndex, item in enumerate(itemsForOctaveBalancing):
                if (itemIndex > 0):
                    print(itemIndex, "Pitch: ", item['pitches'][0], "diff: ", abs(
                        itemsForOctaveBalancing[itemIndex - 1]['pitches'][0] -
                        itemsForOctaveBalancing[itemIndex]['pitches'][0]), item['duration'][0], item['Clk'],
                          item['notes'][0])
                else:
                    print(itemIndex, "Pitch: ", item['pitches'][0], "diff: ", 0, item['duration'][0], item['Clk'],
                          item['notes'][0])
        ...
        for itemIndex, item in enumerate(itemsForOctaveBalancing):
            if (itemIndex == 0):
                ...
                continue
            diff = abs(
                itemsForOctaveBalancing[itemIndex - 1]['pitches'][0] - itemsForOctaveBalancing[itemIndex]['pitches'][0])
            if (diff > 7):

                newItem = copy.deepcopy(item)
                if (itemsForOctaveBalancing[itemIndex - 1]['pitches'][0] >
                        itemsForOctaveBalancing[itemIndex]['pitches'][0]):
                ...
                while (cnt < numItems):
                    if (itemsForOctaveBalancing[cnt]['duration'][0] > checkForNote):
                        ...
                        itemsForOctaveBalancing[cnt]['duration'][0] = itemsForOctaveBalancing[cnt]['Clk'][1] - \
                                                                      itemsForOctaveBalancing[cnt]['Clk'][0]
                        ...
                        for i in range(cnt - 1, itemIndex - 1, -1):
                            ...
                            itemsForOctaveBalancing[i]['duration'][0] = itemsForOctaveBalancing[i]['Clk'][1] - \
                                                                        itemsForOctaveBalancing[i]['Clk'][0]
        ...
        if (0):
            ...
            for itemIndex, item in enumerate(smoothItems):
                if (itemIndex > 0):
                    print(itemIndex, "Pitch: ", item['pitches'][0], "diff: ",
                          abs(smoothItems[itemIndex - 1]['pitches'][0] - smoothItems[itemIndex]['pitches'][0]),
                          item['duration'][0], item['Clk'], item['notes'][0])
                else:
                    print(itemIndex, "Pitch: ", item['pitches'][0], "diff: ", 0, item['duration'][0], item['Clk'],
                          item['notes'][0])
