...
from . import (CreateMidiFiles, Section)
from ..Arranging import ArrangeSections
from ..DevServer import Server
from ..Moods import Mood


# Not sure if these are old imports or not.
...
class Template:
    ...
    def printDetails(self):
        ...
        for mvNum in self.movements:
            ...
            print("Movement: ", mvNum, "Mood: ", self.movements[mvNum]['mood'], "Complexity: ",
                  self.movements[mvNum]['complexity'], "Num Uniq Layers:", sectionsObj.numUniqCPs)
            print()
            print("Arrangement for Movement")
    ...
    def populateSections(self):
        ...
        Server.run(self.movements)  # this will be a json object that will be passed to the server
    ...
    def WriteCompositionSettings(self):
        ...
        for mvNum in self.movements:
            ...
            String = "Movement " + str(mvNum) + " type promo " + "NumSections " + str(numSections) + " Mood " + \
                     self.movements[mvNum]['mood'] + "\n"
            # print ( String )
            fout.write(String)
            ...
            for secId in sectionsObj.sections:
                ...
                String = "SectionNum " + str(secId) + " NumPhrases " + str(numPhrases) + " NumChords " + str(
                    numChordsInPhrase) + " tempo " + str(bpm) + " tse " + str(tse) + " startMNum " + str(
                    startingMNum) + "\n"
                # print ( String )
                fout.write(String)
                phNum = 0
                ...
                for chId in sectionsObj.sections[secId]['chords']:
                    if (chId % numChordsInPhrase == 0):
                        ...
                        String = "PhraseNum " + str(phNum) + " StartClk " + str(
                            sectionsObj.sections[secId]['phrases'][phNum]['globalStartTick']) + " EndClk " + str(
                            sectionsObj.sections[secId]['phrases'][phNum]['globalEndTick']) + " Density " + str(
                            sectionsObj.sections[secId]['phrases'][phNum]['density']) + " Layers " + lyr + "\n"
                        # print ( String )
                        fout.write(String)
                        phNum += 1
    ...
    def replicatePhraseForSections(self):
        ...
        for mvNum in range(self.numMovements):
            midiOutput = CreateMidiFiles.Midi(self.Movement[mvNum], self.uniqLayers[mvNum],
                                              self.LyrTemplateForSections[mvNum])
                    ...
                    if (layer in ['melP', 'piano1', 'rhythmChords', 'strings1', 'strings2']):
                        ...
                        for mnum in self.uniqLayers[mvNum][uniqMelId][layer]['main']:
                            for beatnum in self.uniqLayers[mvNum][uniqMelId][layer]['main'][mnum]:
                                print("\t\t\tMeasure: ", mnum, "Beat: ", beatnum,
                                      self.uniqLayers[mvNum][uniqMelId][layer]['main'][mnum][beatnum])
                            print()
    ...
    def WriteCompositionSettings1(self):
        ...
        for mvNum in range(self.numMovements):
            for uniqMelId in self.uniqLayers[mvNum]:
                ...
                for item in self.uniqLayers[mvNum][uniqMelId][layer]:
                    print("\t\t\tItem: Scale: ", item['scale'], "Chord: ", item['chord'])
                    String = "BassId " + str(uniqMelId) + " BassScale " + item['scale'] + " BassChord " + item[
                        'chord'] + " BassDuration " + str(item['duration'][0]) + "\n"
                    fout.write(String)
        ...
        for mvNum in range(self.numMovements):
            ...
            String = "Movement " + str(self.Movement[mvNum]['id']) + " NumSections " + str(self.Movement[mvNum][
                                                                                               'numSections']) + " Mood " + self.element.getSubMood() + " Element " + self.element.getElement() + " Genre " + self.element.getGenre() + "\n"
            fout.write(String)

            for sec in self.Movement[mvNum]['Sections']:
                String = "SectionNum " + str(sec) + " Type " + self.Movement[mvNum]['Sections'][sec]['type'] + " tse " + \
                         self.Movement[mvNum]['Sections'][sec]['tse'] + " tempo " + str(
                    self.Movement[mvNum]['Sections'][sec]['bpm']) + " NumPhrases " + str(
                    self.Movement[mvNum]['Sections'][sec]['repCount']) + " NumMeasuresPerPhrase " + str(
                    self.Movement[mvNum]['Sections'][sec]['phraseLength']) + " StartingMeasureNum " + str(
                    self.Movement[mvNum]['Sections'][sec]['startMNum']) + " EndingMeasureNum " + str(
                    self.Movement[mvNum]['Sections'][sec]['endMNum']) + "\n"
                fout.write(String)
        ...
        for mvNum in range(self.numMovements):
            for sec in self.Movement[mvNum]['Sections']:
                ...
                for ph in self.CompositionSettingsForMelody[mvNum][sec]:
                    String = "StartofSection " + str(sec) + " PhraseNum " + str(ph) + " Clock " + str(
                        self.CompositionSettingsForMelody[mvNum][sec][ph]['clock']) + " Mute " + str(
                        self.CompositionSettingsForMelody[mvNum][sec][ph]['mute']) + " MaxPitch " + str(
    ...
    def ReadCompositionSettings1(self):
        ...
        for mvNum in compositionSettings:
            print("Movement: ", mvNum, "Mood: ", movementSettings[mvNum]['mood'], "Element: ",
                  movementSettings[mvNum]['element'], "Genre: ", movementSettings[mvNum]['genre'])
            for sec in compositionSettings[mvNum]:
                print("Section: ", sec, "Type: ", movementSettings[mvNum][sec]['type'], "Time Signature: ",
                      movementSettings[mvNum][sec]['tse'], "Tempo: ", movementSettings[mvNum][sec]['tempo'])
                for ph in compositionSettings[mvNum][sec]:
                    print("phrase: ", ph, "Clock: ", compositionSettings[mvNum][sec][ph]['clock'], "Mute: ",
                          compositionSettings[mvNum][sec][ph]['mute'])
