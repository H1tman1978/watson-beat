...
from . import (Constants, Pentatonic, PeruvianRhythmChords, Piano1, RhythmChords, Strings1, Strings2)
...
class AccompanyingLayers:
    ...
    def getChordSplitDuration(self):
        ...
        self.chordSplitDuration[
            durationOneMeasure * 4] = chordSplitDuration + chordSplitDuration + chordSplitDuration + chordSplitDuration
        ...
        for layer in self.layers:
            ...
            if (layer == 'strings2'):
                ...
                for strClass in chordDataForLayer[layer]:
                    for instId in chordDataForLayer[layer][strClass]:
                        if (chordDataForLayer[layer][strClass][instId][0]['desc'] == strClass):
                            ...
                            desc = strClass + "_" + chordDataForLayer[layer][strClass][instId][0][
                                'desc']  # for lostrings, midstrings and histrings
                ...
                for key in self.chord:  # key holds the unique chord durations eg. [1920, 3840] etc
                    ...
                    if (layer == 'melP'):
                        pMel = Pentatonic.PMelody(self.wbLevers, self.scale[key], self.chord[key], self.ids[key],
                                                  self.duration[key])
                        ...
                        if (layer == 'piano1'):
                            piano1 = Piano1.Piano1(self.wbLevers, self.scale[key], self.chord[key], self.ids[key],
                                                   self.duration[key])
                            simple, complex = piano1.run(self.chordSplitDuration[self.duration[key]])
                        elif (layer == 'peruvianRhythmChords'):
                            peruvianRhythmChords = PeruvianRhythmChords.PeruvianRhythmChords(self.wbLevers,
                                                                                             self.scale[key],
                                                                                             self.chord[key],
                                                                                             self.ids[key],
                                                                                             self.duration[key])
                        ...
                        rhyChords = RhythmChords.RhythmChords(self.wbLevers, self.scale[key], self.chord[key],
                                                              self.ids[key], self.duration[key])
                        chordDataForLayer[layer].update(rhyChords.run())

                    elif (layer == 'strings1'):
                        strings1 = Strings1.Strings1(self.wbLevers, self.scale[key], self.chord[key], self.ids[key],
                                                     self.duration[key])
                ...
                if (layer == 'piano1' or layer == 'peruvianRhythmChords'):
                    ...
                    TrainedData[
                        layer] = collections.OrderedDict()  # TrainedData['piano'] = collections.ordereddict[chordIds]  => TrainedData['piano'][0], TrainedData['piano'][1] etc
                    for chordId in range(self.numChords):
                        TrainedData[layer][
                            chordId] = collections.OrderedDict()  # TrainedData['piano'][0,1,2] = collections.ordereddict [simple, complex] => TrainedData['piano'][0]['simple'] , TrainedData['piano'][0]['complex']
                    ...
                    TrainedData[
                        layer] = collections.OrderedDict()  # TrainedData['rhythmChords'] = collections.ordereddict[chordIds]  => TrainedData['rhythmChords'][0], TrainedData['rhythmChords'][1] etc
                    for chordId in range(self.numChords):
