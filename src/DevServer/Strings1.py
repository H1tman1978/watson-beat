...
class Strings1:
    ...
    def __init__(self, wbLevers, scale, chord, ids, duration):
        ...
        self.speed = 'slow'  # why slow for strings always, because we dont want the chords to be split into multiple
        # segments. We want to sustain the chord notes for as long as we can

        # store all chord ids with same duration
        self.chordIds = ids

        # if there is more than one chord with the same duration store them here. Will be used later to fill the
        # actions based on the first learned chord for this duration
    ...
    def run(self):
        ...
        if self.duration in Constants.OtherChordDurationBreakdown:
            ...
            chordSplitDuration = []
            split = Constants.OtherChordDurationBreakdown[self.duration][self.speed][
                0]  # pick the longest duration always, in order to sustain the notes for strings
            for item in split:
        ...
        for item in chordSplitDuration:
            ...
            for note in range(numNotesInChord):
                ...
                eventItems.append({'action': 'RhythmChordTonesStrings1', 'notes': [cNote], 'cNoteIndex': cNoteIndex,
        ...
        for id in range(numChords):
            ...
            for chordSplit in range(len(items)):
                ...
                for item in items[chordSplit]:
                    ...
                    eventItems.append(
                        {'event': 'on', 'action': item['action'], 'notes': [cNote], 'midiClk': item['midiClkStart'],
                         'pitch': midiPitchStr,
                ...
                for item in items[chordSplit]:
                    ...
                    eventItems.append(
                        {'event': 'off', 'action': item['action'], 'notes': [cNote], 'midiClk': item['midiClkEnd'],
                         'pitch': midiPitchStr,
                         'pitches': [pitch], 'chord': chord, 'scale': scale, 'duration': item['duration'],
