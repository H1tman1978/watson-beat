...
class Melody:
        ...
        for trIter in range(trainingIterations):  # called for every training iteration
            ...
            while usedTicks < self.duration:
                ...
                if (
                    ...
                    chosenDuration, chosenNote, totalDuration, velocity, currentNoteIndex = self.getDuration(chosenNote,
                                                                                                             remainingTicks,
                                                                                                             currentNoteIndex)
                    ...
                    chosenData = {'action': chosenAction, 'notes': chosenNote, 'pitches': pitch, 'chord': currentChord,
                                  'scale': currentScale, 'duration': chosenDuration, 'totalDuration': totalDuration,
                                  'velocity': velocity}
                    chosenReward = 0
                    ...
                    if (randForExploration >= exploitationThreshold):
                        ...
                        chosenNote = self.getNextNoteToBePlayed(chosenAction, prevNote, currentScale, currentChord)
                        chosenDuration, chosenNote, totalDuration, velocity, currentNoteIndex = self.getDuration(
                            chosenNote, remainingTicks, currentNoteIndex)

                        pitch = self.getOctavesAndPitchData(chosenNote, prevPitch)
                        # velocity = [ random.randint(50,80) for note in chosenNote ]
                        chosenData = {'action': chosenAction, 'notes': chosenNote, 'pitches': pitch,
                                      'chord': currentChord, 'scale': currentScale, 'duration': chosenDuration,
                        ...
                        for action in actionList:
                            ...
                            currentNote = self.getNextNoteToBePlayed(currentAction, prevNote, currentScale,
                                                                     currentChord)

                            currentDuration, currentNote, totalDuration, velocity, newCurrentNoteIndex = self.getDuration(
                                currentNote, remainingTicks, currentNoteIndex)
                            ...
                            if (initial or q >= qMax):
                                ...
                                chosenData = {'action': currentAction, 'notes': currentNote, 'pitches': pitch,
                                              'chord': currentChord, 'scale': currentScale, 'duration': currentDuration,
                                              'totalDuration': totalDuration, 'velocity': velocity}
    ...
    def fillGapsWithRestNotes(self, items):
        ...
        for item in items:
            ...
            newItem = {'duration': [], 'chord': item['chord'], 'scale': item['scale'], 'action': item['action'],
                       'velocity': [], 'notes': [], 'pitches': [], 'totalDuration': 0}
