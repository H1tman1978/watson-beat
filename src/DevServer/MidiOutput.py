    ...
    for item in data:
        ...
        for note in range(len(item['notes'])):
            ...
            if (item['notes'][note] != 'Rest'):
                ...
                string = "on = midi.NoteOnEvent(tick=" + str(startTick) + ", velocity=" + str(
                    velocity) + ", pitch=midi." + item['notes'][note] + "_" + str(octave) + ")" + "\n";
                fout.write(string);
                fout.write("track.append(on)\n")
                string = "off = midi.NoteOffEvent(tick=" + str(endTick) + ", velocity=" + str(0) + ", pitch=midi." + \
                         item['notes'][note] + "_" + str(octave) + ")" + "\n";
                fout.write(string);
...
def WriteInitialMidiFileSequence(fout, tsInfo):
    ...
    string = "time = midi.TimeSignatureEvent(tick=0, " + "data = [" + str(tsInfo['tsNumerator']) + ", " + str(
