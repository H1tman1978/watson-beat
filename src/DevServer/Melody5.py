...
from . import (Constants, Environment, MusicTheory)
...
class Melody5:
    ...
    def __init__(self, wbLevers, scale, chord, ids, duration):
        ...
        self.Knobs, self.Actions, self.ReverseActions = ComplexityKnobs.setKnobs(self.desc, wbLevers['complexity'],
                                                                                 layerParams)
        self.numActions = len(self.Actions)
        self.defaultAction = 1  # 'PLAY_CHORD_TONE'

        # Create the melody5 environment
        self.env = Environment.Melody5Environment(wbLevers['tse'], wbLevers['phraseLength'], self.homeScale,
                                                  self.homeChord)
    ...
    def initializeQLearningObject(self, rlParams):
        ...
        self.qLearn = QLearning.QLearning(self.numFeatures, self.features, self.numActions, 1024, 0.50, 32, 2, 0.95,
                                          0.20, 1.0, self)
    ...
    def getNextState(self, action, data):  # data will include the note being played and duration
        ...
        if (1):
            gestureMovement = round(float((diffInHomePitchCurrPitch * 1.0) / (24 * 1.0)),
                                    3)  # why 24. we cannot go 2 octaves above homenote
        ...
        for i in range(self.numFeatures):
            ...
            if ('DURATION_CHORD_TONES_FOR_CHORD_LENGTH' in self.StateAttributes and feat == self.StateAttributes[
                'DURATION_CHORD_TONES_FOR_CHORD_LENGTH']):
            ...
            elif ('DURATION_NON_CHORD_TONES_FOR_CHORD_LENGTH' in self.StateAttributes and feat == self.StateAttributes[
                'DURATION_NON_CHORD_TONES_FOR_CHORD_LENGTH']):
            ...
            elif ('HOME_GESTURE_MOVEMENT_DISTANCE' in self.StateAttributes and feat == self.StateAttributes[
                'HOME_GESTURE_MOVEMENT_DISTANCE']):
                gestureMovement = round(float((diffInHomePitchCurrPitch * 1.0) / (24 * 1.0)),
                                        3)  # why 24. we cannot go 2 octaves above homenote
    ...
    def assignCreditOrBlameForAction(self):
        ...
        if (ratioOfChordTones >= self.Knobs['chordToneThresholdLow'] and ratioOfChordTones <= self.Knobs[
            'chordToneThresholdHigh'] and
                ratioOfNonChordTones >= self.Knobs['nonChordToneThresholdLow'] and ratioOfNonChordTones <= self.Knobs[
                    'nonChordToneThresholdHigh']):
            reward = 1.0
        else:
            reward = -1.0
    ...
    def run(self):
        ...
        for trIter in range(trainingIterations):  # called for every training iteration
            ...
            while usedTicks < self.duration:
                ...
                if (
                        usedTicks == 0):  # this is the first note to be played, so choose between one of the chord tones for the chord
                    ...
                    chosenData = {'action': chosenAction, 'notes': chosenNote, 'pitches': pitch,
                                  'chord': self.homeChord, 'scale': self.homeScale, 'duration': chosenDuration,
                                  'totalDuration': totalDuration, 'velocity': velocity}
                    chosenReward = 0
                    ...
                    if (randForExploration >= exploitationThreshold):
                        ...
                        for action in actionList:
                            ...
                            currentNote = self.getNextNoteToBePlayed(currentAction, prevNote, self.homeScale,
                                                                     self.homeChord)
                            ...
                            self.getNextState(currentAction,
                                              {'notes': currentNote, 'duration': currentDuration, 'pitches': pitch})
                            ...
                            if (initial or q >= qMax):
                                ...
                                chosenData = {'action': currentAction, 'notes': currentNote, 'pitches': pitch,
                                              'chord': self.homeChord, 'scale': self.homeScale,
                                              'duration': currentDuration, 'totalDuration': totalDuration,
                                              'velocity': velocity}
                ...
                if (0):
                    print("Used Ticks: ", usedTicks, "remaining duration: ", remainingTicks, "Total Ticks: ",
                          self.duration)
            ...
            if (1):
                ...
                print(self.env.durationChordTonesForChordLength, self.env.durationNonChordTonesForChordLength,
                      self.env.homeGestureMovementDistance)
    ...
    def getRandThreshold(self, trIter, trainingIterations):
        ...
        if (0):
            print("trIter: ", trIter, "Num Iterations: ", trainingIterations, "Ratio: ", ratioIter,
                  "Rand Exploitation: ", randExploitation)
    ...
    def getDuration(self, notes, remainingDuration):
        ...
        if (numNotes == 4):
            if (remainingDuration <= Constants.NoteDurations['0.25'][
                'ticks'] and self.speed == 'medium'):  # less than or equal to quarter
                del notes[2]
                numNotes = len(notes)
            elif (remainingDuration <= Constants.NoteDurations['0.375'][
                'ticks'] and self.speed == 'slow'):  # less than or equal to three-eights
                del notes[1:2]
                numNotes = len(notes)
    ...
    def populateAdditionalChordsOfSameDuration(self, items):
        ...
        for id in range(1, numChords, 1):
            ...
            for item in items:
                ...
                diffInNotes = len(
                    duration) - 2  # why 2, because we will copy the first and last note according to the appropriate action. the rest will be filled out based on the original rhythm learned
                ...
                print("Current Pitch: ", pitch, "Current Notes: ", newNotes, "prevNotes: ", prevNote, "prev Pitches: ",
                      prevPitch, "prev Octave: ", prevOctave)
                ...
                newItem = {'action': action, 'notes': newNotes, 'pitches': pitch, 'chord': chord, 'scale': scale,
    ...
    def AssignRestNotes(self, chosenItems):
        ...
        if (1):
            print("Before Rest Note Duration: ", restNoteDuration, "Probability: ", restNoteProbability,
        ...
        if (1):
            print("After  Rest Note Duration: ", restNoteDuration, "Probability: ", restNoteProbability,
                  "Total Duration: ", self.duration)
        ...
        while (durationLeft > 0):
            ...
            print("Here", chosenItems[pickRandItem1]['notes'], chosenItems[pickRandItem1]['notes'][pickRandItem2],
                  chosenItems[pickRandItem1]['velocity'], chosenItems[pickRandItem1]['velocity'][pickRandItem2])
            ...
            if (chosenItems[pickRandItem1]['duration'][pickRandItem2] <= durationLeft):
                ...
                print("Note: ", chosenItems[pickRandItem1]['notes'][pickRandItem2], "duration Left: ", durationLeft,
                      chosenItems[pickRandItem1]['notes'][pickRandItem2])
                print(chosenItems[pickRandItem1]['notes'], chosenItems[pickRandItem1]['duration'], "velocity:",
                      chosenItems[pickRandItem1]['velocity'])
                print()
                continue
