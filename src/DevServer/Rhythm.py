...
class Rhythm:
    ...
    def setChordDurationTemplates(self, ticksForChord):
        ...
        Let's say the chord is for half measure and the tse of 3/4. Then the duration of the chord is 720 ticks.
        ( 1/4 note = 480 ticks. measure = 480*3. half measure = ( 480 * 3 ) / 2 )
        This function determines the rhythm for this half measure chord.
        ...
        while True:
            ...
            while currTicks < ticksForChord:
                ...
                currNote = random.choice(list(Constants.NoteDurations.keys()))
                    ...
                    if (self.levers['complexity'] == 'super_simple'):  # avoid 16th notes for super simple
                        if (ticks == 120):
                            continue
                              ...
                              'complexity'] == 'simple'):  # if complexity == simple, avoid having only a single 16th note. Always have them in pairs
                        if (ticks == 120):
    ...
    def createRhythmTemplates(self):
        ...
        template[self.oneHalfMeasureTicks]['choices'] = [
            self.halfMeasureChordTemplate + self.halfMeasureChordTemplate + self.halfMeasureChordTemplate,
        ...
        template[self.twoMeasureTicks]['choices'] = [
            self.halfMeasureChordTemplate + self.halfMeasureChordTemplate + self.halfMeasureChordTemplate + self.halfMeasureChordTemplate,
        ...
        template[self.threeMeasureTicks]['choices'] = [
            template[self.fullMeasureTicks]['firstChoice'] + template[self.fullMeasureTicks]['firstChoice'] +
            template[self.fullMeasureTicks]['firstChoice'],
        ...
        template[self.fourMeasureTicks]['choices'] = [
            template[self.fullMeasureTicks]['firstChoice'] + template[self.fullMeasureTicks]['firstChoice'] +
            template[self.fullMeasureTicks]['firstChoice'] + template[self.fullMeasureTicks]['firstChoice'],
            template[self.oneHalfMeasureTicks]['firstChoice'] + template[self.oneHalfMeasureTicks]['firstChoice'] +
            template[self.fullMeasureTicks]['firstChoice'],
        ...
        template[self.fiveMeasureTicks]['choices'] = [
            template[self.twoMeasureTicks]['firstChoice'] + template[self.threeMeasureTicks]['firstChoice'],
    ...
    def printRhythm(self):
        ...
        if (0):
            for key in self.chordTemplate:
                ...
                print(self.chordTemplate[key]['info'], ". Duration in ticks: ", key, "first choice: ",
                      self.chordTemplate[key]['firstChoice'], "first choice ticks: ",
                      self.chordTemplate[key]['firstChoice'])
